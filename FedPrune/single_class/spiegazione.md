...
Nel Federated Unlearning esatto (*Exact Unlearning*), la rimozione di una classe target $c$ viene realizzata rieseguendo l'intero processo di federazione da zero (*Retraining*), escludendo i dati della classe target fin dal primo round. Sebbene garantisca la rimozione dei gradienti relativi alla classe, il retraining comporta un costo computazionale pari a $O(R \cdot K)$ round di comunicazione e addestramento locale.
...
* `features[0]` (primo blocco convoluzionale: 32 filtri)
* `features[3]` (secondo blocco convoluzionale: 64 filtri)

### 2.2 Pruning Selettivo e Azzeramento del Classificatore
Definito un parametro di potatura $\rho$ (`prune_ratio = 0.3`):
1. **Filtri Convoluzionali:** Vengono individuati i primi $\lfloor K \cdot \rho \rfloor$ canali con il punteggio di sensibilità $CS_{k, c}$ più elevato all'interno di `features[0]` e `features[3]`, e i relativi pesi e bias vengono azzerati:
   $$W_{\text{features}}[k] \leftarrow 0, \quad b_{\text{features}}[k] \leftarrow 0$$
2. **Neurone Finale di Classe:** La riga corrispondente alla classe target $c$ all'interno dell'ultimo strato lineare `classifier[2]` viene azzerata sia nei pesi che nei bias:
...
* Si avvia un ciclo federato compatto di soli **3 round** di fine-tuning con un learning rate dimezzato (`FL_LR * 0.5`).
* Durante questi round, i client si addestrano **esclusivamente sui campioni delle 9 classi trattenute**.
...
Normalizzando rispetto alla somma totale su tutte le classi:
$$v_{\text{sum}} = \sum_{j=0}^{9} \Delta w_j, \quad b_{\text{sum}} = \sum_{j=0}^{9} \Delta b_j$$

Viene calcolato lo **Score di Discriminazione $S_d[i]$** con $\beta = 0.5$:
$$S_d[i] = \beta \frac{\Delta w_i}{v_{\text{sum}}} + (1 - \beta) \frac{\Delta b_i}{b_{\text{sum}}}$$

La classe predetta dall'attacco corrisponde all'indice con lo score massimo:
$$\hat{c} = \arg\max_i S_d[i]$$
...
| **Numero di Round FU** | 50 – 80 round completi | **3 round** di fine-tuning rapido |
